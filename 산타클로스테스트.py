...
# 기본 UI
# -----------------------------
st.set_page_config(page_title="🎅 산타 테스트", page_icon="🎅", layout="centered")

st.title("🎅산타클로스의 착한 아이 테스트")
st.markdown("---")
# -----------------------------
# BGM 설정
#-----------------------------
...
# 배경 설정
# ------------------------------
st.set_page_config(page_title=":산타: 산타 테스트", page_icon=":산타:", layout="centered")
...
/* 중앙 정렬 유지 + 폭 확장 */
div.block-container {
    max-width: 1100px;  /* 기존 720px → 1100px로 확장 */
    padding-left: 2rem;
    padding-right: 2rem;
}
...
if st.session_state.page == "main":
    ...
    st.header("산타클로스의 테스트를 통과해 선물을 받으세요! 🎁")

    _, c, _ = st.columns([1, 2, 1])
    with c:
        st.image("Santa.png", width=650)

    st.markdown("### 🎄 산타가 부를 당신의 이름은?")
...
# -----------------------------
    ...
    st.markdown(f"## 🎄 스테이지 {int(row['stage'])} · {row['title']}")
...
# 결과 페이지
    ...
    if result_row.empty:
        st.error(f"점수 {score}점에 해당하는 결과가 없습니다.")
        st.stop()

    result = result_row.iloc[0]
    name = st.session_state.name

    st.markdown(f"### ⭐ **{name}**은(는) **{result['result_title']}**")
    st.markdown(f"### 총점: **{score}점**")
